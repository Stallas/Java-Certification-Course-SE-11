...
/* we get compiler error throws CustomThrowable in method signature. result same if Throwable or CustomException
  because we cannot declare subclasses i.e. customException directly extends Exception.
  customThrowable is a subclass of Throwable class.
  Throwable is a parent of Exception and customException extends it so declaring throwable is fine.
 */
...
//            cex.throwThrowable(0); exception handled in catch block
            cex.testExceptionNotHandled(0);
        } catch (CustomException c) { // CustomThrowable is never thrown from the try block so compiler error.
            c.printStackTrace(System.out);
        }

        System.out.println("---- Custom Exception Thrown/Not Caught -----");
//        cex.throwThrowable(0);// It should handle exception in method signature throws Throwable by declaring. else compiler-error
