...
class CollegeStudent {
    // initially, the datatype is Float results in nullptr exception when printstatus is called.
    // best practice is Optional should never be null
...
public class OptionalTests {
    public static void main(String[] args) {
        ...
        List.of(joe, jane)
                .stream()
                .filter(s -> s.getGpa().isPresent())// can also use .isEmpty()
...
//                                " on academic probation")));
        List.of(joe, jane)
                ...
                .forEach(s -> System.out.println(s.getName() +
                        (s.getGpa()
//                                 .orElse(2.0f)
//                                .orElse(OptionalTests.getGpaEstimate())// invoked for both jane and joe
//                                .orElseThrow()
                                .orElseGet(OptionalTests::getGpaEstimate)// invoked only for jane
                                >= 2.0f
                                ? " in good standing" :
                                " on academic probation")));
